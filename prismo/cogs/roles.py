...
EMOJIS: Dict[str, Tuple[str, str, int]] = {
    "announcements": ("🚀", "Get pinged for server announcements!", 874016049165651988),
    "releases": ("📢", "Get pinged for new releases!", 874016317508841512),
}

#939724049330868274 #940841684802105355
class Roles(commands.Cog):
    def __init__(self, bot: PrismoBot) -> None:
        self.bot = bot

    @commands.command()
    async def create_roles_message(self, ctx: commands.Context, create: bool, channel: disnake.TextChannel) -> None:
        embed = disnake.Embed(
            title='Roles',
        ...
        if create:
            ...
            message = self.bot.get_message(MESSAGE_ID) or await channel.fetch_message(MESSAGE_ID)
        ...
        for key, (emoji, _, role_id) in EMOJIS.items():
            ...
            if interaction.component.emoji == disnake.PartialEmoji.from_str(emoji):
                ...
                if not isinstance(interaction.author, disnake.Member):
                    self.bot.logger.debug("Got a User object when expecting a Member object")
                ...
                if not role_id in [role.id for role in interaction.author.roles]:
                    ...
                    self.bot.logger.info("Added %s (id: %s) role to %s", key, role_id, interaction.author)
                ...
                self.bot.logger.info("Removed %s (id: %s) role to %s", key, role_id, interaction.author)
